...
fn solve_part_one(serial_number: u32) -> (usize, usize) {
  solve(serial_number, PART_ONE_SQUARE_SIZE, PART_ONE_SQUARE_SIZE).0
}

fn solve_part_two(serial_number: u32) -> ((usize, usize), usize) {
  solve(serial_number, 1, FUEL_CELL_GRID_SIZE - 1)
}
...
fn solve(
  ...
  if max_square_size >= FUEL_CELL_GRID_SIZE {
    panic!("max square size must be less than grid size");
  }
  let power_levels: Vec<Vec<_>> = (1..=FUEL_CELL_GRID_SIZE as u32)
    .map(|y| {
      (1..=FUEL_CELL_GRID_SIZE as u32)
  ...
  for square_size in 1..=max_square_size {
    let max_index = FUEL_CELL_GRID_SIZE - square_size;
