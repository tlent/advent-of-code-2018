...
impl World {
  ...
  fn simulate_water(&mut self) {
    ...
    while !downward_stack.is_empty() {
      ...
      while !horizontal_stack.is_empty() {
        ...
        let valid_horizontal: Vec<_> = vec![(current_x - 1, current_y), (current_x + 1, current_y)]
          .into_iter()
          .filter(|p| self.filled_tiles.get(p).is_none() && self.is_valid_position(*p))
          .collect();
  ...
  fn count_water_and_flow(&self) -> u32 {
    ...
    self
      ...
      .iter()
      .filter(|((_x, y), t)| {
        *y >= min_clay_bound && *y <= max_clay_bound && (**t == Tile::Water || **t == Tile::Flow)
      })
      .count() as u32
  }
  ...
  fn count_water(&self) -> u32 {
    ...
    self
      .filled_tiles
      .iter()
      .filter(|((_x, y), t)| *y >= min_clay_bound && *y <= max_clay_bound && (**t == Tile::Water))
      .count() as u32
  }
